...
// The OCR cache is an INPUT to the build, not an output, so it must not live
// inside _corpus/ -- build-corpus wipes that directory on every run and would
// delete the cache it depends on, silently reverting every transcribed page to
// a stub on the next build. Caught by build-corpus --check, which is what that
// check is for.
const OCR_CACHE = join(ROOT, "_data", "ocr-cache.json");
...
/**
 * OCR transcriptions produced by the reviewed pass in scripts/ocr-pass.mjs.
 * Builds CONSUME this cache and never run OCR, so the corpus is reproducible and
 * an engine version bump cannot silently rewrite the record.
 */
function loadOcrCache() {
  if (!existsSync(OCR_CACHE)) return { pages: {} };
  return JSON.parse(readFileSync(OCR_CACHE, "utf8"));
}

// Header line marking a page as machine-transcribed. It is deliberately blunt
// and sits directly above the text, so a retrieved chunk carries the warning
// with it rather than leaving it behind on a page the model never sees.
function ocrNotice(confidence) {
  return [
    `MACHINE TRANSCRIPTION (OCR), confidence ${confidence}%. NOT A QUOTABLE SOURCE.`,
    "",
    "This page of the filing is a scanned image with no text layer. The text below",
    "was produced by optical character recognition and contains transcription",
    "errors: OCR of these documents interleaves vertical margin text into body",
    "lines. Use it to find and describe what the filing covers. Do NOT quote it as",
    "the words of the document; open the linked PDF for the actual text.",
  ].join("\n");
}
...
function main() {
  ...
  for (const doc of docs) {
    ...
    pages.forEach((raw, i) => {
      ...
      const hasNativeText = body.length >= MIN_PAGE_CHARS;

      // No native text layer: fall back to a reviewed OCR transcription if one
      // was accepted for this page, otherwise the explicit stub.
      const ocrKey = `${doc.slug}/p${String(pageNo).padStart(3, "0")}`;
      const ocr = hasNativeText ? null : ocrCache.pages?.[ocrKey];
      const usableOcr = ocr && ocr.usable && ocr.text ? ocr : null;

      const textSource = hasNativeText ? "native" : usableOcr ? "ocr" : "none";
      let pageText;
      if (hasNativeText) {
        pageText = body;
        docTextPages += 1;
        ...
        pageText = ocrNotice(usableOcr.confidence) + "\n\n" + usableOcr.text;
        docTextPages += 1;
        ocrPages += 1;
        totalChars += usableOcr.text.length;
      } else {
        pageText = ocr
          ? NO_TEXT_STUB +
            `\n\nOCR was attempted on this page and the result was not trusted ` +
            `(confidence ${ocr.confidence}%, below the ${ocrCache.engine?.min_confidence ?? 60}% threshold).`
          : NO_TEXT_STUB;
        noTextPages += 1;
      }
      ...
      const content =
        ...
        (usableOcr ? ` (OCR, confidence ${usableOcr.confidence}%, not quotable)` : "") +
        "\n\n---\n\n" +
        pageText +
        "\n";
      ...
      manifest.push({
        ...
        has_text: textSource !== "none",
        // "native" text may be quoted. "ocr" may NOT: verify-citations refuses a
        // quoted citation to an OCR page. "none" is a scan with no usable
        // transcription at all.
        text_source: textSource,
        ocr_confidence: usableOcr ? usableOcr.confidence : null,
      });
    });
  ...
  const searchable = textPages + ocrPages;
  const coverage = {
    generated_from: "scripts/build-corpus.mjs",
    extractor: "pdftotext -layout (poppler)",
    ocr_engine: ocrCache.engine ?? null,
    documents: docs.length,
    total_pages: totalPages,
    // Native text is quotable. OCR text is searchable but NOT quotable.
    pages_native_text: textPages,
    pages_ocr_text: ocrPages,
    pages_with_text: searchable,
    pages_without_text: noTextPages,
    coverage_pct: Number(((searchable / totalPages) * 100).toFixed(1)),
    native_coverage_pct: Number(((textPages / totalPages) * 100).toFixed(1)),
  ...
  console.log(`Native text:      ${coverage.pages_native_text} (quotable)`);
  console.log(`OCR text:         ${coverage.pages_ocr_text} (searchable, NOT quotable)`);
  console.log(`Searchable total: ${coverage.pages_with_text} (${coverage.coverage_pct}%)`);
  console.log(`No usable text:   ${coverage.pages_without_text}`);
